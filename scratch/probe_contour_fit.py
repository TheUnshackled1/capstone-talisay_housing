"""Contour-based shaded lot fit (follow drawing, not AABB)."""
from pathlib import Path

import cv2
import numpy as np
from PIL import Image, ImageDraw

ROOT = Path(__file__).resolve().parent.parent
rgb = np.array(Image.open(ROOT / "static/images/lot_plan_roads.png").convert("RGB"))
gray = cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY)
H, W = gray.shape

ink = (gray < 108).astype(np.uint8) * 255
ink = cv2.dilate(ink, np.ones((3, 3), np.uint8), 2)
shaded = ((gray >= 125) & (gray <= 185)).astype(np.uint8) * 255
cells = cv2.bitwise_and(shaded, cv2.bitwise_not(ink))
cells = cv2.morphologyEx(cells, cv2.MORPH_OPEN, np.ones((2, 2), np.uint8), 1)

seeds = [
    (108.0, 96.0), (138.0, 101.0), (167.0, 106.0), (196.0, 111.0),
    (104.0, 116.0), (133.0, 126.0), (161.0, 134.0), (194.0, 145.0),
]


def order_quad(pts):
    arr = np.array(pts, dtype=float)
    ang = np.arctan2(arr[:, 1] - arr[:, 1].mean(), arr[:, 0] - arr[:, 0].mean())
    return [pts[int(j)] for j in np.argsort(ang)]


def fit_cell(sx, sy):
    ix, iy = int(round(sx)), int(round(sy))
    if cells[iy, ix] == 0:
        found = False
        for r in range(1, 10):
            for dy in range(-r, r + 1):
                for dx in range(-r, r + 1):
                    xx, yy = ix + dx, iy + dy
                    if 0 <= xx < W and 0 <= yy < H and cells[yy, xx]:
                        ix, iy, found = xx, yy, True
                        break
                if found:
                    break
            if found:
                break
    if cells[iy, ix] == 0:
        return None
    mask = np.zeros((H + 2, W + 2), np.uint8)
    flood = cells.copy()
    cv2.floodFill(flood, mask, (ix, iy), 200)
    cell = (flood == 200).astype(np.uint8) * 255
    cell = cv2.dilate(cell, np.ones((2, 2), np.uint8), 1)
    contours, _ = cv2.findContours(cell, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        return None
    cnt = max(contours, key=cv2.contourArea)
    if cv2.contourArea(cnt) < 80:
        return None
    peri = cv2.arcLength(cnt, True)
    approx = cv2.approxPolyDP(cnt, 0.04 * peri, True)
    if len(approx) < 4:
        # fall back to minAreaRect
        rect = cv2.minAreaRect(cnt)
        box = cv2.boxPoints(rect).astype(np.float32)
    else:
        # take 4 strongest corners: if more than 4, use convex hull then approx again
        hull = cv2.convexHull(cnt)
        approx = cv2.approxPolyDP(hull, 0.03 * cv2.arcLength(hull, True), True)
        if len(approx) == 4:
            box = approx.reshape(-1, 2).astype(np.float32)
        elif len(approx) > 4:
            # pick 4 extrema by angle around centroid via minAreaRect still,
            # but clip to hull points that are farthest along rect axes
            rect = cv2.minAreaRect(cnt)
            box = cv2.boxPoints(rect).astype(np.float32)
            # project hull onto box corners — keep box but shrink harder
        else:
            rect = cv2.minAreaRect(cnt)
            box = cv2.boxPoints(rect).astype(np.float32)

    c = box.mean(axis=0)
    # shrink enough to stay inside thick black borders
    box = c + (box - c) * 0.88
    return order_quad([(float(x), float(y)) for x, y in box]), c


img = Image.fromarray(rgb)
dr = ImageDraw.Draw(img)
for sx, sy in seeds:
    hit = fit_cell(sx, sy)
    if not hit:
        print("fail", sx, sy)
        continue
    poly, c = hit
    dr.line(poly + [poly[0]], fill=(0, 220, 80), width=2)
    print(f"({sx:.0f},{sy:.0f}) n={len(poly)} center=({c[0]:.1f},{c[1]:.1f})")

out = ROOT / "scratch" / "shaded_contour_fit.png"
img.crop((90, 70, 230, 180)).save(out)
print("wrote", out)
