...
bak = Path("scratch/lot_plan_polygons_before_circle_cut_196.json")
...
# print current geometry + find meeting region
allp = []
for i in IDS:
...
x0 = int(allp[:, 0].mean())
y0 = int(allp[:, 1].mean())
# better: mean of each lot's vertex nearest overall centroid
...
print("inner corners mean", CX0, CY0, "inners", inners)

# refine circle: bright core + dark ring near meeting point
best = None
...
def bevel_inner(xy):
    """Replace vertex nearest circle with 3-point arc on that lot's side."""
    n = len(xy)
...
).save("scratch/fix_196_circle_cut.png")
