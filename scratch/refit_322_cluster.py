"""Trace and refit lots 322, 332, 344, 353 — 4-way junction circle excluded."""
import json
import math
import shutil
...
bak = Path("scratch/lot_plan_polygons_before_refit_322_cluster.json")
...
# detect circle at junction
best = None
for cy in range(383, 398):
    for cx in range(290, 305):
        ang = np.linspace(0, 2 * math.pi, 48, endpoint=False)
        ring = np.array(
            [gray[int(cy + 6 * math.sin(a)), int(cx + 6 * math.cos(a))] for a in ang]
        ).mean()
        core = gray[cy - 2 : cy + 3, cx - 2 : cx + 3].mean()
        score = core - ring
        if best is None or score > best[0]:
            best = (score, cx, cy)
CX, CY, R = float(best[1]), float(best[2]), 9.0
print("circle", CX, CY, "score", best[0])

LOTS = {
    322: {"seed": (285, 365), "roi": (268, 352, 302, 382), "inner": "se"},
    332: {"seed": (315, 370), "roi": (302, 352, 328, 390), "inner": "sw"},
    344: {"seed": (285, 390), "roi": (268, 378, 302, 408), "inner": "ne"},
    353: {"seed": (315, 400), "roi": (302, 388, 328, 418), "inner": "nw"},
}
...
def quad_for(inner: str, x0: float, y0: float, x1: float, y1: float) -> np.ndarray:
    if inner == "se":  # 322 top-left lot, inner corner bottom-right
        return np.array([[x0, y0], [x1, y0], [x1, y1], [x0, y1]], float)
    if inner == "sw":  # 332 top-right, inner bottom-left
        return np.array([[x0, y0], [x1, y0], [x1, y1], [x0, y1]], float)
    if inner == "ne":  # 344 bottom-left, inner top-right
        return np.array([[x0, y0], [x1, y0], [x1, y1], [x0, y1]], float)
    # nw — 353 bottom-right, inner top-left
    return np.array([[x0, y0], [x1, y0], [x1, y1], [x0, y1]], float)
...
def merge_raw(xy: np.ndarray, raw: np.ndarray) -> np.ndarray:
    ...
    for i, target in enumerate(out):
        for p in raw:
            if np.linalg.norm(p - target) < 3.0:
                out[i] = p
                break
    return out
...
for idx in IDS:
    ...
    xy = quad_for(cfg["inner"], x0, y0, x1, y1)
    xy = merge_raw(xy, raw)
    xy = bevel_inner(xy)
    lots[idx] = {
        "points": to_norm(xy),
...
vis.crop((int(CX) - 50, int(CY) - 45, int(CX) + 50, int(CY) + 45)).resize(
    (700, 650), Image.NEAREST
).save("scratch/fix_322_cluster_trace.png")
