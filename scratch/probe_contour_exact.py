"""Contour-exact shaded fits (SVG can use N-gons)."""
...
def fit_contour(sx, sy):
    ...
    # grow slightly toward borders so hover covers the drawn gray pad
    ...
    c = pts.mean(axis=0)
    pts = c + (pts - c) * 0.96  # tiny inset so stroke stays inside black ink
    return [(float(x), float(y)) for x, y in pts], c


img = Image.fromarray(rgb)
dr = ImageDraw.Draw(img)
for sx, sy in seeds:
    hit = fit_contour(sx, sy)
    if not hit:
        print("fail", sx, sy)
        continue
    poly, c = hit
    dr.line(poly + [poly[0]], fill=(0, 220, 80), width=2)
    print(f"({sx:.0f},{sy:.0f}) verts={len(poly)} c=({c[0]:.1f},{c[1]:.1f})")

out = ROOT / "scratch" / "shaded_contour_exact.png"
